...
void BOARD_InitClocks(void)
{
    ...
    (void) CCM_RootSetParent(CLOCK_ROOT_GLITCHFILTER, CLOCK_SRC_FRO);
    (void) CCM_RootSetRate(CLOCK_ROOT_GLITCHFILTER, ES_24MHZ,
        CLOCK_ROUND_RULE_CEILING);
}
...
void BOARD_InitHandlers(void)
{
    ...
    NVIC_EnableIRQ(GPIO1_INT0_IRQn);
}
...
void BOARD_InitTimers(void)
{
    ...
    /* Halt CM7_0 WDOG on CM7_0 debug entry */
    BLK_CTRL_WAKEUPMIX->IPG_DEBUG_CM7_0 =
        BLK_CTRL_WAKEUPMIX_IPG_DEBUG_CM7_0_WDOG5_MASK;

    /* Halt CM7_1 WDOG on CM7_1 debug entry */
    BLK_CTRL_WAKEUPMIX->IPG_DEBUG_CM7_1 =
        BLK_CTRL_WAKEUPMIX_IPG_DEBUG_CM7_1_WDOG6_MASK;
}
