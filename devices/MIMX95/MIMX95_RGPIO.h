 ...
 * @version 1.0
 * @date 2021-11-16
 * @brief CMSIS Peripheral Access Layer for MIMX95_RGPIO
 *
 * CMSIS Peripheral Access Layer for MIMX95
 */

/* Prevention from multiple including the same memory map */
#if !defined(MIMX95_RGPIO_H_)  /* Check if memory map has not been already included */
#define MIMX95_RGPIO_H_
...
typedef struct
{
    ...
    __IO uint32_t ICR[32]; /**< Interrupt Control Register 0..Interrupt Control Register 31, array offset: 0x80, array
                              step: 0x4 */
 ...
 *       bit fields is only allowed by software in secure state. When the corresponding pin's registers is accessed
 ...
 *       and bit fields is only allowed by software in non-secure state. When the corresponding pin's registers is
...
/*! NSE0 - Non-Secure Enable
 *  0b0..The interrupt/DMA request/trigger output is configured for secure access. Only software in secure state
 *       can configure a pin to use the corresponding interrupt/DMA request/trigger output or reconfigure a pin that
 *       is already configured to use the corresponding interrupt/DMA request/trigger output.
 *  0b1..The interrupt/DMA request/trigger output is configured for non-secure access. Only software in non-secure
 *       state can configure a pin to use the corresponding interrupt/DMA request/trigger output or reconfigure a
 *       pin that is already configured to use the corresponding interrupt/DMA request/trigger output.
 */
 ...
 *  0b0..Corresponding field of PDOR[PDOn] does not change.
 *  0b1..Corresponding field of PDOR[PDOn] is set to the inverse of its existing logic state.
 */
...
#endif  /* #if !defined(MIMX95_RGPIO_H_) */
